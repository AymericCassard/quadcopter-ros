...
def generate_launch_description():
    ...
    # Adds the model paths to the gazebo models env var
    set_gz_resource_path = SetEnvironmentVariable(
        name="GZ_SIM_RESOURCE_PATH",
        value=os.path.join(pkg_project_description, "models")
        + ":"
    ...
    gz_sim = IncludeLaunchDescription(
        ...
        launch_arguments={
            "gz_args": PathJoinSubstitution([pkg_project_gazebo, "worlds", "harmonic.sdf"])
        }.items(),
    )
    ...
    rviz = Node(
        package="rviz2",
        executable="rviz2",
        # TODO: Create rviz conf file and add here
        ...
        parameters=[
            {
                "config_file": os.path.join(
                    pkg_project_bringup, "config", "ros_gz_X3_bridge.yaml"
                ),
